...
from bokeh.models.widgets import CheckboxButtonGroup, TableColumn, DataTable, Button, Dropdown
...
def genGraphRenderer( G, data, Shape ):
    ...
    BaseShape = partial( Shape, fill_color='colors', size='shape_size', fill_alpha='visible', line_alpha='visible' )

    graph_renderer.node_renderer.glyph = BaseShape()
    graph_renderer.node_renderer.selection_glyph = BaseShape()
...
class BokehState():
    ...
    def updateCurrentPossibleLatentStates( self, new_pls ):
        if( np.allclose( np.array( new_pls, dtype=int ), self.pls[ self.current_node ] ) ):
            return False
...
def bokehPlot( doc, pedigree, fbs ):
    ...
    pedigree_fig = figure( width=1000, height=600, tools='pan,wheel_zoom,reset,tap,box_select', x_range=Range1d( x_min, x_max ), y_range=Range1d( y_min, y_max ) )
    ...
    XX_col = TableColumn( field='prob_XX', title='XX', formatter=NumberFormatter( format='0.00' ) )
    Xx_col = TableColumn( field='prob_Xx', title='Xx', formatter=NumberFormatter( format='0.00' ) )
    xx_col = TableColumn( field='prob_xx', title='xx', formatter=NumberFormatter( format='0.00' ) )
    XY_col = TableColumn( field='prob_XY', title='XY', formatter=NumberFormatter( format='0.00' ) )
    xY_col = TableColumn( field='prob_xY', title='xY', formatter=NumberFormatter( format='0.00' ) )

    # Add the possible states box
    button_title = Button( label='Possible latent states' )

    checkbox = CheckboxButtonGroup( labels=[ 'XX', 'Xx', 'xx', 'XY', 'xY' ] )

    # Create the table that will display all of the nodes and their expected probabilities
    data_table = DataTable( columns=[ XX_col, Xx_col, xx_col, XY_col, xY_col ], width=600 )
    ...
    weights, = bokeh_state.ad_params.transition_dist.params
    hinton1 = genHintonDiagram( weights )

    weights, = bokeh_state.ad_params.emission_dist.params
    hinton2 = genHintonDiagram( weights )

    node_specific = column( button_title, checkbox, data_table )

    doc.add_root( row( node_specific, pedigree_fig, hinton1, hinton2 ) )
