...
def testGraphCategoricalForwardBackward():
    ...
    for n, probs in msg.jointParents( U, V, msg.nodes ):
        parents, parent_order = msg.getFullParents( n, get_order=True, full_indexing=True, return_full_indexing=True )
...
def testGraphCategoricalForwardBackwardImproved():
    ...
    msg = GraphCategoricalForwardBackwardFBSImproved()
    msg.updateParamsFromGraphs( ys, initial_dists, transition_dists, emission_dist, graphs )
...
def graphMarginalizationTest():
    ...
    testGraphCategoricalForwardBackwardImproved()
    # assert 0