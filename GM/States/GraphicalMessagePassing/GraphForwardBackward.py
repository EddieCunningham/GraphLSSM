...
__all__ = [ 'GraphCategoricalForwardBackward',
            ...
            'GraphCategoricalForwardBackwardFBSImproved' ]
...
class GraphCategoricalForwardBackwardFBS( _fowardBackwardMixin, GraphFilterFBS ):
    ...
    def transitionProb( self, node ):
        parents, parent_order = self.getFullParents( node, get_order=True )
        ndim = len( parents ) + 1
        pi = self.pis[ ndim ]
        # Reshape pi's axes to match parent order
        assert len( parents ) + 1 == pi.ndim
        ...
        fbsIndices = [ fbsOffset( parent ) for parent in parents if self.inFeedbackSet( parent, from_reduced=True ) ]
        ...
        if( len( fbsIndices ) > 0 ):
            ...
            for i, parent in enumerate( parents ):
                if( self.inFeedbackSet( parent, from_reduced=True ) ):
    ...
    @classmethod
    def integrate( cls, integrand, axes, ignore_fbs_axis=False ):
...
class GraphCategoricalForwardBackwardFBSImproved( _fowardBackwardMixin, GraphFilterFBSImproved ):
    ...
    def transitionProb( self, node, is_partial_graph_index=False ):
        ...
        if( self.inFeedbackSet( node, is_partial_graph_index=is_partial_graph_index ) ):
            fbsIndices.append( self.fbsIndex( node, is_partial_graph_index=is_partial_graph_index, within_graph=True ) + 1 )
