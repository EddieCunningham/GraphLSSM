...
class TensorNormal( TensorExponentialFam ):
    ...
    def log_partition( cls, x=None, params=None, natParams=None, split=False ):
        ...
        A2 = 0.5 * TensorExponentialFam.combine( ( M, M ), cls.invs( covs ) )
    ...
    def sample( cls, params=None, natParams=None, size=1 ):
        ...
        N = len( shapes )
        totalDim = np.prod( shapes )

        X = Normal.sample( D=1, size=totalDim ).reshape( shapes )

        ind1 = string.ascii_letters[ : N ]
        ind2 = string.ascii_letters[ N : N * 2 ]
        contract = ind1 + ',' + ','.join( [ a + b for a, b in zip( ind1, ind2 ) ] ) + '->' + ind2
    ...
    def log_likelihoodRavel( cls, x, params=None, natParams=None ):
        ...
        return Normal.log_likelihood( x.ravel(), params=( fullMu, fullCov ) )
    ...
    def log_likelihood( cls, x, params=None, natParams=None ):
        ...
        return -0.5 * TensorExponentialFam.combine( ( x - M, x - M ), covInvs )+ \
               -0.5 * sum( [ totalDim / cov.shape[ 0 ] * np.linalg.slogdet( cov )[ 1 ] for cov in covs ] ) + \
